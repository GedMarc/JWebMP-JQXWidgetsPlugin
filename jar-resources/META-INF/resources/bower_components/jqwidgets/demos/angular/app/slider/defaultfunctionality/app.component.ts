...
export class AppComponent {
    ...
    tooltipFormatFunction: any = (value) => {
        if (this.redSlider.mode() === 'default') {
            if (value < 10) return Number(value.toPrecision(3));
            if (value > 10 && value < 100) return Number(value.toPrecision(4));
            else return Number(value.toPrecision(5));
        }
        else {
            if (value < 10) return Number(value.toPrecision(3));
            if (value > 10 && value < 256) return Number(value.toPrecision(4));
        }
    };
